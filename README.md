...
- **The route was optimized for a 10-second timeout budget that, it turned out, doesn't actually apply to this project.** Vercel's legacy Hobby limit was a hard 10s regardless of `maxDuration`; since Fluid Compute became the default in June 2026, Hobby's default function budget is 300s (5 minutes), and this project (created after that change) uses it — confirmed empirically by deploying a route that sleeps 15s and watching it return 200 rather than time out. The round-trip optimizations below (bulk insert, batched embedding calls, one schema-check transaction) are still good practice and were kept, but they were solving a tighter constraint than actually exists here. Worth re-checking for your own deployment: Fluid Compute can be disabled on older projects, which would bring back the strict 10s cap.
